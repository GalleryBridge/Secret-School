...
public class PointsBoardPersistentHandler {
    ...
    public void savePointsBoard2DB(){
        try {
            ...
            // 2.计算动态表名
            // 2.1.查询赛季信息
            Integer season = seasonService.querySeasonByTime(time);      //5
            // 2.2.存入ThreadLocal
            TableInfoContext.setInfo(POINTS_BOARD_TABLE_PREFIX + season);   //points_board_5

            // 3.查询榜单数据
            // 3.1.拼接KEY
            String key = RedisConstants.POINTS_BOARD_KEY_PREFIX + time.format(DateUtils.POINTS_BOARD_SUFFIX_FORMATTER);     //boards:202305
            // 3.2.查询数据
            ...
            while (true) {
                ...
                // 4.2.持久化
                pointsBoardService.saveBatch(boardList);            //points_board_5
                // 5.翻页
                pageNo+=total;   //pageNo = pageNo + 2;
            }
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
